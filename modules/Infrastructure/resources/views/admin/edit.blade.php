...
<div class="card">
  <div class="card-header">
    <h4 class="card-title mb-0">Infrastructure Edit ({{ $infrastructure->title }})</h4>
  </div><!-- end card header -->
  ...
  <form action="{{ route('infrastructure.update', $infrastructure->id) }}" method="post" enctype="multipart/form-data">
    ...
    <div class="card-body">
      ...
      <div class="mb-3">
        ...
        <input type="text" name="title" id="title" class="form-control" value="{{ $infrastructure->title }}">
        <x-form.validation.error name="title" />
      </div>

      <div class="mb-3">
        <p class="text-muted">Content</p>
        <textarea name="body" id="body" class="form-control" rows="10">{{ $infrastructure->title }}</textarea>
